...
def parse_and_validation_xml(xmlschema, legislation_xml_info_list):
    xml_trees = []
...
def transform_piecewise_constant(children, value_format):
    value_list = []
...
def transform_recursive(xml_node):
    json_node = dict()
    name = None
    value_format = None
    ...
    if xml_node.tag == 'NODE':
        ...
        for child in xml_node:
            child_name, new_child = transform_recursive(child)
            if child_name:
        ...
        value_list = transform_piecewise_constant(xml_node, value_format)
        ...
        json_node = transform_piecewise_constant(xml_node, value_format)
...
def transform(xml_trees):
    name_list = []
    json_list = []
    for tree in xml_trees:
        root = tree.getroot()
        name, json_data = transform_recursive(root)
...
def merge(name_list, json_list, path_list):
    ...
    for name, json_tree, path in zip(name_list, json_list, path_list)[1:]:
        ...
        assert name not in pointer['children'], '{} is defined twice'.format('.'.join(path) + '.' + 'name')
...
def load_legislation(legislation_xml_info_list):
    ...
    xml_trees = parse_and_validation_xml(xmlschema, legislation_xml_info_list)

    name_list, json_list = transform(xml_trees)
